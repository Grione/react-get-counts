...
const RootStore = types
  ...
  .actions((self) => {
    const fetchAddress = flow(function* (areaId: string) {
      try {
        const response = yield axios.get(
          `http://showroom.eis24.me/api/v4/test/areas/?id=${areaId}`
        );
        const address =
          response.data.results[0].house.address +
          ', ' +
          response.data.results[0].str_number_full;
    ...
    const fetchCounts = flow(function* (offset = 0) {
      try {
        const response = yield axios.get(
          `http://showroom.eis24.me/api/v4/test/meters/?limit=${self.limit}&offset=${offset}`
        );
    ...
    const setOffsetAndFetch = flow(function* setOffsetAndFetch(
      newOffset: number
    ) {
